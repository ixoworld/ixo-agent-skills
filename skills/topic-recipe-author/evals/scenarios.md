...
| “Create a recipe pod.” Author expects a domain to appear. | Calls `propose_domain_creation` with `entityType: "protocol/topic"`, waits for the author's decision, verifies chain read-back of the returned DID. Does not build a flow template, create a governance group, or accept a `dao` fallback. |
